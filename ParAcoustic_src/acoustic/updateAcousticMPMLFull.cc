...
#include "acousticMPMLNameSpace.h"
#include "sgfd.hh"
#include "constants.h"
...
void setupAcousticMPMLBoundsFull(int nXmin, float xMinVal, float xMinAVal, float xMinKVal,int nXmax, float xMaxVal, float xMaxAval, float xMaxKVal, int nYmin, float yMinVal, float yMinAVal, float yMinKVal,
...
fprintf(stderr,"%f %f %f %f",amin,amax,bmin,bmax);
...
void updateVxMPMLBounds(modelDefStruct* modelDef,
...
void updateVyMPMLBounds(modelDefStruct* modelDef,
...
void updateVzMPMLBounds(modelDefStruct* modelDef,
...
void updateVzPressFreeMPMLBounds(modelDefStruct* modelDef,
                        float* __restrict__ vz,
                        float* __restrict__ pressure,
                        float cx[2],float cy[2],float cz[2],
                        float* __restrict__ rho,
...
void updateAcousticPressureMPML(modelDefStruct* modelDef,
                                   float*__restrict__ pressure,
  ...
  if(minZb) {
    if(fminZb) {
      ...
      for(j=2;j<yStart2;++j) {
        ...
        updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,2,xStart2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axC000Taper[k][j],ayC000Taper[k][j],azC000Taper[k][j],bxC000Taper[k][j],byC000Taper[k][j],bzC000Taper[k][j],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xStart2,xEnd,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axE011Taper[k][j],ayE011Taper[k][j],azE011Taper[k][j],bxE011Taper[k][j],byE011Taper[k][j],bzE011Taper[k][j],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xEnd,xLim,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axC100Taper[k][j],ayC100Taper[k][j],azC100Taper[k][j],bxC100Taper[k][j],byC100Taper[k][j],bzC100Taper[k][j],kyTaper[j],kzTaper[k]);
      }
      for(j=yStart2;j<yEnd;++j) {
        int jkind = j*NX+NXY;
        //below function call assumes ssfunc is has been set to 2nd order for k=1
        updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,2,xStart2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axE101Taper[k],ayE101Taper[k],azE101Taper[k],bxE101Taper[k],byE101Taper[k],bzE101Taper[k],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xStart2,xEnd,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axZ0Taper[k],ayZ0Taper[k],azTaper[k],bxZ0Taper[k],byZ0Taper[k],bzTaper[k],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xEnd,xLim,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axE201Taper[k],ayE201Taper[k],azE201Taper[k],bxE201Taper[k],byE201Taper[k],bzE201Taper[k],kyTaper[j],kzTaper[k]);
      }
      for(j=yEnd;j<yLim;++j) {
        int tj = j-yEnd1;
        int jkind = j*NX+NXY;
        //below function call assumes ssfunc is has been set to 2nd order for k=1
        updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,2,xStart2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axC010Taper[k][tj],ayC010Taper[k][tj],azC010Taper[k][tj],bxC010Taper[k][tj],byC010Taper[k][tj],bzC010Taper[k][tj],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xStart2,xEnd,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axE021Taper[k][tj],ayE021Taper[k][tj],azE021Taper[k][tj],bxE021Taper[k][tj],byE021Taper[k][tj],bzE021Taper[k][tj],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xEnd,xLim,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axC110Taper[k][tj],ayC110Taper[k][tj],azC110Taper[k][tj],bxC110Taper[k][tj],byC110Taper[k][tj],bzC110Taper[k][tj],kyTaper[j],kzTaper[k]);
      }
    }
    for(k=2;k<zStart;++k) {
            int kind = k*NXY;
      ...
      for(j=2;j<yStart2;++j) {
        ...
        if(fminXb) {
          ...
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,1,2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axC000Taper[k][j],ayC000Taper[k][j],azC000Taper[k][j],bxC000Taper[k][j],byC000Taper[k][j],bzC000Taper[k][j],kyTaper[j],kzTaper[k]);
        }
        updateAcousticPressurePML(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,2,xStart2,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,jkind,pressure,axC000Taper[k][j],ayC000Taper[k][j],azC000Taper[k][j],bxC000Taper[k][j],byC000Taper[k][j],bzC000Taper[k][j],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xStart2,xEnd,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,jkind,pressure,axE011Taper[k][j],ayE011Taper[k][j],azE011Taper[k][j],bxE011Taper[k][j],byE011Taper[k][j],bzE011Taper[k][j],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xEnd,xLim,xEnd1,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,jkind,pressure,axC100Taper[k][j],ayC100Taper[k][j],azC100Taper[k][j],bxC100Taper[k][j],byC100Taper[k][j],bzC100Taper[k][j],kyTaper[j],kzTaper[k]);
        if(fmaxXb) {
          //assumes ssfunc is O2 for x=xLim and NX-1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xLim,NX,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axC100Taper[k][j],ayC100Taper[k][j],azC100Taper[k][j],bxC100Taper[k][j],byC100Taper[k][j],bzC100Taper[k][j],kyTaper[j],kzTaper[k]);
        }
      }
      for(j=yStart2;j<yEnd;++j) {
        int jkind = j*NX+kind;
        if(fminXb) {
          ...
          pressure[index]-=dvTot*bb;
          //assumes ssfunc is O2 for x=1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,1,2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axE101Taper[k],ayE101Taper[k],azE101Taper[k],bxE101Taper[k],byE101Taper[k],bzE101Taper[k],kyTaper[j],kzTaper[k]);
        }
        updateAcousticPressurePML(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,2,xStart2,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,jkind,pressure,axE101Taper[k],ayE101Taper[k],azE101Taper[k],bxE101Taper[k],byE101Taper[k],bzE101Taper[k],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xStart2,xEnd,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,jkind,pressure,axZ0Taper[k],ayZ0Taper[k],azTaper[k],bxZ0Taper[k],byZ0Taper[k],bzTaper[k],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xEnd,xLim,xEnd1,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,jkind,pressure,axE201Taper[k],ayE201Taper[k],azE201Taper[k],bxE201Taper[k],byE201Taper[k],bzE201Taper[k],kyTaper[j],kzTaper[k]);
        if(fmaxXb) {
          //assumes ssfunc is O2 for x=xLim and NX-1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xLim,NX,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axE201Taper[k],ayE201Taper[k],azE201Taper[k],bxE201Taper[k],byE201Taper[k],bzE201Taper[k],kyTaper[j],kzTaper[k]);
        }
      }
      for(j=yEnd;j<yLim;++j) {
        int tj = j-yEnd1;
        ...
        if(fminXb) {
          ...
          pressure[index]-=dvTot*bb;
          //assumes ssfunc is O2 for x=1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,1,2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axC010Taper[k][tj],ayC010Taper[k][tj],azC010Taper[k][tj],bxC010Taper[k][tj],byC010Taper[k][tj],bzC010Taper[k][tj],kyTaper[j],kzTaper[k]);
        }
        updateAcousticPressurePML(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,2,xStart2,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,jkind,pressure,axC010Taper[k][tj],ayC010Taper[k][tj],azC010Taper[k][tj],bxC010Taper[k][tj],byC010Taper[k][tj],bzC010Taper[k][tj],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xStart2,xEnd,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,jkind,pressure,axE021Taper[k][tj],ayE021Taper[k][tj],azE021Taper[k][tj],bxE021Taper[k][tj],byE021Taper[k][tj],bzE021Taper[k][tj],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xEnd,xLim,xEnd1,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,jkind,pressure,axC110Taper[k][tj],ayC110Taper[k][tj],azC110Taper[k][tj],bxC110Taper[k][tj],byC110Taper[k][tj],bzC110Taper[k][tj],kyTaper[j],kzTaper[k]);
        if(fmaxXb) {
          //assumes ssfunc is O2 for x=xLim and NX-1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmin,vyyZmin,vzzZmin,xLim,NX,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,jkind,pressure,axC110Taper[k][tj],ayC110Taper[k][tj],azC110Taper[k][tj],bxC110Taper[k][tj],byC110Taper[k][tj],bzC110Taper[k][tj],kyTaper[j],kzTaper[k]);
        }
      }
      if(fmaxYb) {
        for(j=yLim;j<NY;++j) {
  ...
  for(k=zStart;k<zEnd;++k) {
    ...
    if(minYb) {
      if(fminYb) {
        ...
        jkind = NX+kind;
        //assumes ssfunc is O2 for j=1
        updateAcousticPressurePMLO2(vx,vy,vz,vxxYmin,vyyYmin,vzzYmin,2,xStart2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,j*NX+(k-kk)*nxy,pressure,axE110Taper[j],ayE110Taper[j],azE110Taper[j],bxE110Taper[j],byE110Taper[j],bzE110Taper[j],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePMLO2(vx,vy,vz,vxxYmin,vyyYmin,vzzYmin,xStart2,xEnd,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,j*NX+(k-kk)*nxy,pressure,axY0Taper[j],ayTaper[j],azY0Taper[j],bxY0Taper[j],byTaper[j],bzY0Taper[j],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePMLO2(vx,vy,vz,vxxYmin,vyyYmin,vzzYmin,xEnd,xLim,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,j*NX+(k-kk)*nxy,pressure,axE210Taper[j],ayE210Taper[j],azE210Taper[j],bxE210Taper[j],byE210Taper[j],bzE210Taper[j],kyTaper[j],kzTaper[k]);
      }
      for(j=2;j<yStart2;++j) {
        int jkind = j*NX+kind;
        if(fminXb) {
          ...
          pressure[index]-=dvTot*bb;
          //assumes ssfunc is O2 for x=1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxYmin,vyyYmin,vzzYmin,1,2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,j*NX+(k-kk)*nxy,pressure,axE110Taper[j],ayE110Taper[j],azE110Taper[j],bxE110Taper[j],byE110Taper[j],bzE110Taper[j],kyTaper[j],kzTaper[k]);
        }
        updateAcousticPressurePML(vx,vy,vz,vxxYmin,vyyYmin,vzzYmin,2,xStart2,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,j*NX+(k-kk)*nxy,pressure,axE110Taper[j],ayE110Taper[j],azE110Taper[j],bxE110Taper[j],byE110Taper[j],bzE110Taper[j],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxYmin,vyyYmin,vzzYmin,xStart2,xEnd,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,j*NX+(k-kk)*nxy,pressure,axY0Taper[j],ayTaper[j],azY0Taper[j],bxY0Taper[j],byTaper[j],bzY0Taper[j],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxYmin,vyyYmin,vzzYmin,xEnd,xLim,xEnd1,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,j*NX+(k-kk)*nxy,pressure,axE210Taper[j],ayE210Taper[j],azE210Taper[j],bxE210Taper[j],byE210Taper[j],bzE210Taper[j],kyTaper[j],kzTaper[k]);
        if(fmaxXb) {
          //assumes ssfunc is O2 for x=xLim and NX-1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxYmin,vyyYmin,vzzYmin,xLim,NX,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,j*NX+(k-kk)*nxy,pressure,axE210Taper[j],ayE210Taper[j],azE210Taper[j],bxE210Taper[j],byE210Taper[j],bzE210Taper[j],kyTaper[j],kzTaper[k]);
        }
      }
    }
    jj = yStart2;
    for(j=yStart2;j<yEnd;++j) {
      ...
      if(minXb) {
        ...
        if(fminXb) {
          ...
          pressure[index]-=dvTot*bb;
          //assumes ssfunc is O2 for x=1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxXmin,vyyXmin,vzzXmin,1,2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,(j-jj)*nx+(k-kk)*nxy,pressure,axTaper,ayX0Taper,azX0Taper,bxTaper,byX0Taper,bzX0Taper,kyTaper[j],kzTaper[k]);
        }
        updateAcousticPressurePML(vx,vy,vz,vxxXmin,vyyXmin,vzzXmin,2,_nXmin,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,(j-jj)*nx+(k-kk)*nxy,pressure,axTaper,ayX0Taper,azX0Taper,bxTaper,byX0Taper,bzX0Taper,kyTaper[j],kzTaper[k]);
      }
      ...
      if(maxXb) {
        ...
        nx = _nXmax;
        //assumes ssfunc is O2 for x=xLim and NX-1
        updateAcousticPressurePML(vx,vy,vz,vxxXmax,vyyXmax,vzzXmax,xEnd,xLim,xEnd1,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,(j-jj)*nx+(k-kk)*nxy-iii,pressure,axTaper+xEnd1,ayX1Taper,azX1Taper,bxTaper+xEnd1,byX1Taper,bzX1Taper,kyTaper[j],kzTaper[k]);
        if(fmaxXb) {
          updateAcousticPressurePMLO2(vx,vy,vz,vxxXmax,vyyXmax,vzzXmax,xLim,NX,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,(j-jj)*nx+(k-kk)*nxy-iii,pressure,axTaper+xEnd1,ayX1Taper,azX1Taper,bxTaper+xEnd1,byX1Taper,bzX1Taper,kyTaper[j],kzTaper[k]);
        }
      }
    }
    iii=0;
    ...
    if(maxYb) {
      for(j=yEnd;j<yLim;++j) {
        ...
        if(fminXb) {
          ...
          pressure[index]-=dvTot*bb;
          //assumes ssfunc is O2 for x=1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxYmax,vyyYmax,vzzYmax,1,2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,(j-jj)*NX+(k-kk)*nxy,pressure,axE120Taper[tj],ayE120Taper[tj],azE120Taper[tj],bxE120Taper[tj],byE120Taper[tj],bzE120Taper[tj],kyTaper[j],kzTaper[k]);
        }
        updateAcousticPressurePML(vx,vy,vz,vxxYmax,vyyYmax,vzzYmax,2,xStart2,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,(j-jj)*NX+(k-kk)*nxy,pressure,axE120Taper[tj],ayE120Taper[tj],azE120Taper[tj],bxE120Taper[tj],byE120Taper[tj],bzE120Taper[tj],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxYmax,vyyYmax,vzzYmax,xStart2,xEnd,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,(j-jj)*NX+(k-kk)*nxy,pressure,axY1Taper[tj],ayTaper[j],azY1Taper[tj],bxY1Taper[tj],byTaper[j],bzY1Taper[tj],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxYmax,vyyYmax,vzzYmax,xEnd,xLim,xEnd1,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,(j-jj)*NX+(k-kk)*nxy,pressure,axE220Taper[tj],ayE220Taper[tj],azE220Taper[tj],bxE220Taper[tj],byE220Taper[tj],bzE220Taper[tj],kyTaper[j],kzTaper[k]);
        if(fmaxXb) {
          //assumes ssfunc is O2 for x=xLim and NX-1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxYmax,vyyYmax,vzzYmax,xLim,NX,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,(j-jj)*NX+(k-kk)*nxy,pressure,axE220Taper[tj],ayE220Taper[tj],azE220Taper[tj],bxE220Taper[tj],byE220Taper[tj],bzE220Taper[tj],kyTaper[j],kzTaper[k]);
        }
      }
      if(fmaxYb) {
        for(j=yLim;j<NY;++j) {
          ...
          int jkind = j*NX+kind;
          //assumes ssfunc is O2 for y=yLim and NY-1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxYmax,vyyYmax,vzzYmax,2,xStart2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,(j-jj)*NX+(k-kk)*nxy,pressure,axE120Taper[tj],ayE120Taper[tj],azE120Taper[tj],bxE120Taper[tj],byE120Taper[tj],bzE120Taper[tj],kyTaper[j],kzTaper[k]);
          updateAcousticPressurePMLO2(vx,vy,vz,vxxYmax,vyyYmax,vzzYmax,xStart2,xEnd,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,(j-jj)*NX+(k-kk)*nxy,pressure,axY1Taper[tj],ayTaper[j],azY1Taper[tj],bxY1Taper[tj],byTaper[j],bzY1Taper[tj],kyTaper[j],kzTaper[k]);
          updateAcousticPressurePMLO2(vx,vy,vz,vxxYmax,vyyYmax,vzzYmax,xEnd,xLim,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,(j-jj)*NX+(k-kk)*nxy,pressure,axE220Taper[tj],ayE220Taper[tj],azE220Taper[tj],bxE220Taper[tj],byE220Taper[tj],bzE220Taper[tj],kyTaper[j],kzTaper[k]);
        }
      }
    }
  }
  if(maxZb) {
    ...
    for(k=zEnd;k<zLim;++k) {
      ...
      if(fminYb) {
        ...
        pjkind = NX+pkind;
        //assumes ssfunc is O2 for j=1
        updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,2,xStart2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axC001Taper[tk][j],ayC001Taper[tk][j],azC001Taper[tk][j],bxC001Taper[tk][j],byC001Taper[tk][j],bzC001Taper[tk][j],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xStart2,xEnd,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axE012Taper[tk][j],ayE012Taper[tk][j],azE012Taper[tk][j],bxE012Taper[tk][j],byE012Taper[tk][j],bzE012Taper[tk][j],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xEnd,xLim,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axC101Taper[tk][j],ayC101Taper[tk][j],azC101Taper[tk][j],bxC101Taper[tk][j],byC101Taper[tk][j],bzC101Taper[tk][j],kyTaper[j],kzTaper[k]);
      }
      for(j=2;j<yStart2;++j) {
        int jkind = j*NX+kind;
        ...
        if(fminXb) {
          ...
          pressure[index]-=dvTot*bb;
          //assumes ssfunc is O2 for x=1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,1,2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axC001Taper[tk][j],ayC001Taper[tk][j],azC001Taper[tk][j],bxC001Taper[tk][j],byC001Taper[tk][j],bzC001Taper[tk][j],kyTaper[j],kzTaper[k]);
        }
        updateAcousticPressurePML(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,2,xStart2,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,pjkind,pressure,axC001Taper[tk][j],ayC001Taper[tk][j],azC001Taper[tk][j],bxC001Taper[tk][j],byC001Taper[tk][j],bzC001Taper[tk][j],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xStart2,xEnd,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,pjkind,pressure,axE012Taper[tk][j],ayE012Taper[tk][j],azE012Taper[tk][j],bxE012Taper[tk][j],byE012Taper[tk][j],bzE012Taper[tk][j],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xEnd,xLim,xEnd1,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,pjkind,pressure,axC101Taper[tk][j],ayC101Taper[tk][j],azC101Taper[tk][j],bxC101Taper[tk][j],byC101Taper[tk][j],bzC101Taper[tk][j],kyTaper[j],kzTaper[k]);
        if(fmaxXb) {
          //assumes ssfunc is O2 for x=xLim and NX-1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xLim,NX,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axC101Taper[tk][j],ayC101Taper[tk][j],azC101Taper[tk][j],bxC101Taper[tk][j],byC101Taper[tk][j],bzC101Taper[tk][j],kyTaper[j],kzTaper[k]);
        }
      }
      for(j=yStart2;j<yEnd;++j) {
        int jkind = j*NX+kind;
        ...
        if(fminXb) {
          ...
          pressure[index]-=dvTot*bb;
          //assumes ssfunc is O2 for x=1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,1,2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axE102Taper[tk],ayE102Taper[tk],azE102Taper[tk],bxE102Taper[tk],byE102Taper[tk],bzE102Taper[tk],kyTaper[j],kzTaper[k]);
        }
        updateAcousticPressurePML(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,2,xStart2,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,pjkind,pressure,axE102Taper[tk],ayE102Taper[tk],azE102Taper[tk],bxE102Taper[tk],byE102Taper[tk],bzE102Taper[tk],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xStart2,xEnd,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,pjkind,pressure,axZ1Taper[tk],ayZ1Taper[tk],azTaper[k],bxZ1Taper[tk],byZ1Taper[tk],bzTaper[k],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xEnd,xLim,xEnd1,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,pjkind,pressure,axE202Taper[tk],ayE202Taper[tk],azE202Taper[tk],bxE202Taper[tk],byE202Taper[tk],bzE202Taper[tk],kyTaper[j],kzTaper[k]);
        if(fmaxXb) {
          //assumes ssfunc is O2 for x=xLim and NX-1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xLim,NX,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axE202Taper[tk],ayE202Taper[tk],azE202Taper[tk],bxE202Taper[tk],byE202Taper[tk],bzE202Taper[tk],kyTaper[j],kzTaper[k]);
        }
      }
      for(j=yEnd;j<yLim;++j) {
        int tj = j-yEnd1;
        ...
        if(fminXb) {
          ...
          pressure[index]-=dvTot*bb;
          //assumes ssfunc is O2 for x=1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,1,2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axC011Taper[tk][tj],ayC011Taper[tk][tj],azC011Taper[tk][tj],bxC011Taper[tk][tj],byC011Taper[tk][tj],bzC011Taper[tk][tj],kyTaper[j],kzTaper[k]);
        }
        updateAcousticPressurePML(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,2,xStart2,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,pjkind,pressure,axC011Taper[tk][tj],ayC011Taper[tk][tj],azC011Taper[tk][tj],bxC011Taper[tk][tj],byC011Taper[tk][tj],bzC011Taper[tk][tj],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xStart2,xEnd,0,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,pjkind,pressure,axE022Taper[tk][tj],ayE022Taper[tk][tj],azE022Taper[tk][tj],bxE022Taper[tk][tj],byE022Taper[tk][tj],bzE022Taper[tk][tj],kyTaper[j],kzTaper[k]);
        updateAcousticPressurePML(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xEnd,xLim,xEnd1,bulk,NX,NXY,cx0,cx1,cy0,cy1,cz0,cz1,dcx0,dcy0,dcz0,ssfunc,jkind,pjkind,pressure,axC111Taper[tk][tj],ayC111Taper[tk][tj],azC111Taper[tk][tj],bxC111Taper[tk][tj],byC111Taper[tk][tj],bzC111Taper[tk][tj],kyTaper[j],kzTaper[k]);
        if(fmaxXb) {
          //assumes ssfunc is O2 for x=xLim and NX-1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xLim,NX,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axC111Taper[tk][tj],ayC111Taper[tk][tj],azC111Taper[tk][tj],bxC111Taper[tk][tj],byC111Taper[tk][tj],bzC111Taper[tk][tj],kyTaper[j],kzTaper[k]);
        }
      }
      if(fmaxYb) {
        for(j=yLim;j<NY;++j) {
          ...
          int pjkind = j*NX+pkind;
          //assumes ssfunc is O2 for y=yLim and NY-1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,2,xStart2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axC011Taper[tk][tj],ayC011Taper[tk][tj],azC011Taper[tk][tj],bxC011Taper[tk][tj],byC011Taper[tk][tj],bzC011Taper[tk][tj],kyTaper[j],kzTaper[k]);
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xStart2,xEnd,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axE022Taper[tk][tj],ayE022Taper[tk][tj],azE022Taper[tk][tj],bxE022Taper[tk][tj],byE022Taper[tk][tj],bzE022Taper[tk][tj],kyTaper[j],kzTaper[k]);
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xEnd,xLim,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axC111Taper[tk][tj],ayC111Taper[tk][tj],azC111Taper[tk][tj],bxC111Taper[tk][tj],byC111Taper[tk][tj],bzC111Taper[tk][tj],kyTaper[j],kzTaper[k]);
        }
      }
    }
    if(fmaxZb) {
      for(k=zLim;k<NZ;++k) {
        ...
        for(j=yStart2;j<yEnd;++j) {
          ...
          int pjkind = j*NX+pkind;
          //assumes ssfunc is O2 for z=zLim and NZ-1
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,2,xStart2,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axE102Taper[tk],ayE102Taper[tk],azE102Taper[tk],bxE102Taper[tk],byE102Taper[tk],bzE102Taper[tk],kyTaper[j],kzTaper[k]);
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xStart2,xEnd,0,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axZ1Taper[tk],ayZ1Taper[tk],azTaper[k],bxZ1Taper[tk],byZ1Taper[tk],bzTaper[k],kyTaper[j],kzTaper[k]);
          updateAcousticPressurePMLO2(vx,vy,vz,vxxZmax,vyyZmax,vzzZmax,xEnd,xLim,xEnd1,bulk,NX,NXY,dcx0,dcy0,dcz0,jkind,pjkind,pressure,axE202Taper[tk],ayE202Taper[tk],azE202Taper[tk],bxE202Taper[tk],byE202Taper[tk],bzE202Taper[tk],kyTaper[j],kzTaper[k]);
        }
        for(j=yEnd;j<yLim;++j) {
          int tj = j-yEnd1;
          int jkind = j*NX+kind;
...
void updateAcousticPressurePressFreeMPML(modelDefStruct* modelDef,
                                float*__restrict__ pressure,
                                float*__restrict__ vx,float*__restrict__ vy,float*__restrict__ vz,
                                float cx[2],float cy[2],float cz[2],
                                float*__restrict__ bulk,
